...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(){let activeImage=$(".gallery img.gallery-item[src='"+$(".lightboxImage").attr("src")+"']");let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".gallery img.gallery-item").each(function(){imagesCollection.push($(this).attr("src"))})}else{$(".gallery img.gallery-item[data-gallery-tag='"+activeTag+"']").each(function(){imagesCollection.push($(this).attr("src"))})}
let index=imagesCollection.indexOf(activeImage.attr("src"));if(index>0){let prev=imagesCollection[index-1];$(".lightboxImage").attr("src",prev)}},nextImage(){let activeImage=$(".gallery img.gallery-item[src='"+$(".lightboxImage").attr("src")+"']");let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".gallery img.gallery-item").each(function(){imagesCollection.push($(this).attr("src"))})}else{$(".gallery img.gallery-item[data-gallery-tag='"+activeTag+"']").each(function(){imagesCollection.push($(this).attr("src"))})}
let index=imagesCollection.indexOf(activeImage.attr("src"));if(index<imagesCollection.length-1){let next=imagesCollection[index+1];$(".lightboxImage").attr("src",next)}},createLightBox(gallery,lightboxId,navigation){gallery.append(`<div class="modal fade" id="${lightboxId ? lightboxId : "galleryLightbox"
